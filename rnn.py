...
def rnn(x):
    ...
    rnn_cell = tf.nn.rnn_cell.BasicRNNCell(num_units=n_hidden)
...
with tf.Session() as sess:
    ...
    for step in range(1, n_training_step + 1):
        ...
        if (step % 10 == 0) or (step == n_training_step):
            optimized_cost, training_acc = sess.run([cost_op, acc], feed_dict={
                X: batch_x, Y: batch_y})
...
# Danh gia do chinh xac cua mo hinh
    ...
    test_data = mnist.test.images[:256].reshape((-1, n_time_step, n_input))
